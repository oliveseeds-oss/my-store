...
// POST /api/digital-inquiries — Rate-limited public digital project inquiry
router.post("/", createRateLimiter(5, 15 * 60 * 1000), async (req, res) => {
  ...
  try {
    ...
    console.error("Failed to submit digital inquiry:", err);
    res.status(500).json({ error: "Failed to submit digital inquiry." });
  }
});
...
router.get("/admin/all", verifyAdmin, async (req, res) => {
  try {
    ...
    console.error("Failed to fetch digital inquiries:", err);
